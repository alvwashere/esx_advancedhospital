						...
						if success then
							...
							if IsEntityDead(ESX.PlayerData.ped) then
								if Config.oxLib then
									TriggerServerEvent('esx_ambulancejob:revive', cache.serverId)
								else
									TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
								end
							end
							SetEntityCoords(ESX.PlayerData.ped, 314.4377, -584.4855, 44.2039)
							SetEntityHeading(ESX.PlayerData.ped, 68.1603)
							...
							if Config.dpEmotes then
								...
								TaskPlayAnim(ESX.PlayerData.ped, 'timetable@tracy@sleep@', 'idle_c', 2.0, 2.0, -1, 0, 0, false, false, false)
							end
							...
							if Config.oxLib and Config.progressBar then
								lib.progressBar({
									duration = 15000,
									label = 'The doctors are helping you...',
									useWhileDead = false,
									canCancel = false,
									disable = {
										car = true,
							...
							SetEntityCoords(ESX.PlayerData.ped, 308.3181, -586.9536, 43.2841)
							DoScreenFadeIn(500)
